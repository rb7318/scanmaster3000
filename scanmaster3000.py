...
def plat_sync(start, finish):
    ...
    min_current = controller.min_current.get()
    max_current = controller.max_current.get()
    psync = controller.psync.get()
    multiple_plat_filter = controller.multiple_plat_filter.get()
    #Save current directory (should be at script level)
    original_directory = os.getcwd()
    #Make new directoy to work in:
    ...
    try:
        ...
        for i in range(start-1, finish):
            ...
            plat_end = 0
            save_flag = False
            if multiple_plat_filter:
                plats = 0
                for j in range(len(plat_avg)):
                    plat_num = plat_crd[j]
                    if ((plat_num==1) and (plat_avg[j] > min_current) and (plat_avg[j] < max_current)):
                        print "Plateau number", plat_num, "is in range, with value", plat_avg[j]
                        plats=plats+1
                    elif ((plat_num==2) and (plat_avg[j] > 2*min_current) and (plat_avg[j] < 2*max_current)):
                        print "Plateau number", plat_num, "is in range, with value", plat_avg[j]
                        plats=plats+1
                    elif ((plat_num==3) and(plat_avg[j] > 3*min_current) and (plat_avg[j] < 3*max_current)):
                        print "Plateau number", plat_num, "is in range, with value", plat_avg[j]
                        plats=plats+1
                    elif ((plat_num==4) and(plat_avg[j] > 4*min_current) and (plat_avg[j] < 4*max_current)):
                        print "Plateau number", plat_num, "is in range, with value", plat_avg[j]
                        plats=plats+1
                if plats>=3:
                    save_flag = True
                ...
                for j in range(len(plat_avg)):
                    if ( (plat_avg[j] > min_current) and (plat_avg[j] < max_current) and (plat_start == 0)):
            ...
            if save_flag == True:
                ...
                if psync:
                    ...
                    fileoutput(filename, 
                            data.i_dat_all_ls[i][:],
                            data.i10_dat_all_ls[i][:],
                            data.i_dat_all_hs[i][:],
                            data.i10_dat_all_hs[i][:],
                            )
                    print "File saved as:", filename
                    filecounter += 1
    finally:
...
class egraph:
    ...
    def plotGaussian(self):
        ...
        self.g_ax.plot(self.x, self.fit4, 'y-', label='Fit 4', linewidth=2)
...
# Change to units of G0       
 ...
 # PLOT 1
        ...
        self.ax.plot(s_dat_ls, i_dat_ls_G0, 'k.', label='LS x1')
        self.ax.plot(s_dat_ls, i10_dat_ls_G0, 'b.', label='LS x10')
        self.ax.plot(s_dat_ls, i_dat_hs_G0, 'g.', label='HS x1')
        self.ax.plot(s_dat_ls, i10_dat_hs_G0, 'r.', label='HS x10')
        self.ax.grid(True)
        ...
        self.ax2.set_ylim([-4, 2])
        self.ax2.set_xlabel('Distance (nm)')
        self.ax2.legend()

        # PLOT 3
        self.ax3.cla()  # Clear current axes
        self.ax3.plot(s_dat_ls, i_dat_ls_G0, 'k.-', label='LS x1')
        self.ax3.plot(s_dat_ls, plat_data_G0, 'b.-', label='Plateau fitting')
        self.ax3.grid(True)
...
class controller:
    ...
    def __init__(self, myParent):
        ...
        self.partial_hist = IntVar()
        self.partial_hist_no = IntVar()
        ...
        self.min_current = DoubleVar()
        self.max_current = DoubleVar()
        self.psync = IntVar()
        self.multiple_plat_filter = IntVar()
        ...
        try: 
            ...
            self.partial_hist.set(0) 
            self.partial_hist_no.set(5)       
            ...
            self.gF_mu4.set(1.77)
            self.gF_sigma4.set(0.20)
            self.gF_scale4.set(0.031)
            ...
            self.min_current.set(7400)
            self.max_current.set(8000)
            self.psync.set(0)
            self.multiple_plat_filter.set(0)
            ...
            try:
                ...
                self.partial_hist.set(data['partial_hist'])
                self.partial_hist_no.set(data['partial_hist_no'])
                ...
                self.min_current.set(data['min_current']) 
                self.max_current.set(data['max_current']) 
                self.psync.set(data['psync']) 
                self.multiple_plat_filter.set(data['multiple_plat_filter'])
    ...
    def saveData(self):
        ...
        data = {'lowres' : self.lowres.get(),
                ...
                'min_current' : self.min_current.get(),
                'max_current' : self.max_current.get(),
                'psync' : self.psync.get(),
                'multiple_plat_filter' : self.multiple_plat_filter.get(),
    ...
    def kde_params(self):
        ...
        Label(self.kde_frame, text='Plot partial histograms?:').grid(row=15,
                column=0, sticky=W)
        self.peakMax = Checkbutton(self.kde_frame, variable=self.partial_hist)
        self.peakMax.grid(row=15, column=1)

        Label(self.kde_frame, text='Partial histogram number').grid(row=16,
                column=0, sticky=W)
        self.peakStop = Spinbox(
            self.kde_frame,
            from_=-10,
            ...
            wrap=True,
            validate='all',
            textvariable=self.partial_hist_no,
            )
        self.peakStop.grid(row=16, column=1)
    ...
    def plateau_fitting_params(self):
        ...
        self.ybin = Spinbox(
            ...
            wrap=True,
            validate='all',
            textvariable=self.min_current,
            )
        self.ybin.grid(row=6, column=1)
        ...
        self.ybin.grid(row=7, column=1)

        Label(self.plateau_fitting_frame, text='Psync data?:').grid(row=8,
                column=0, sticky=W)
        self.ybin = Checkbutton(self.plateau_fitting_frame, variable=self.psync)
        self.ybin.grid(row=8, column=1)

        Label(self.plateau_fitting_frame, text='Filter for multiple plateaus?:').grid(row=9,
                column=0, sticky=W)
        self.ybin = Checkbutton(self.plateau_fitting_frame, variable=self.multiple_plat_filter)
        self.ybin.grid(row=9, column=1)
    ...
    def kdePlot(self, savefig=False):
        ...
        if partial_hist:
            list_i_list=[]
            numScans = len(data.i_dat_all_combined)    
            scanLists = partial_hist_no
            scansPerList = numScans / scanLists
            #Make some current lists
            for j in range(scanLists):
                scanList = data.i_dat_all_combined[(scansPerList*j):(scansPerList*(j+1))]
                i_list = []
            ...
            for scanList in list_i_list:
                i = float(float(scanList[1])/float(scanLists-1))
                colour = [float(1-i),0,float(i)]
                print 'KDE ',scanList[1]+1,' calculated'
                kde = statistics.pdf(scanList[0], x, h=kde_bandwidth, kernel='E')
                plt.plot(x, kde, label='KDE '+str(scanList[1]+1),color=colour, linewidth=1)
